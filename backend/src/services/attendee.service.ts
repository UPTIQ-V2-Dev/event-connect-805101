...
import { Attendee, Prisma } from '../generated/prisma/index.js';
import ApiError from '../utils/ApiError.ts';
import httpStatus from 'http-status';

// Type for formatted attendee response
export interface AttendeeWithGuestInfo {
    id: string;
    eventId: string;
    name: string;
    email: string;
 ...
 */
const getEventAttendees = async (eventId: string): Promise<AttendeeWithGuestInfo[]> => {
    // Check if event exists
    const event = await prisma.event.findUnique({
        where: { id: eventId }
    });
 ...
 * Get attendee by ID
 */
const getAttendeeById = async (attendeeId: string): Promise<AttendeeWithGuestInfo | null> => {
 ...
 */
const deleteAttendeeById = async (attendeeId: string): Promise<AttendeeWithGuestInfo> => {
 ...
 */
function formatAttendeeWithGuestInfo(attendee: Attendee): AttendeeWithGuestInfo {
    return {
        id: attendee.id,
